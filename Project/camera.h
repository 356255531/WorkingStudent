...
private:
	...
	bool Init(int width, int height, int frame_rate, bool do_argb_conversion);
	void Release();
...
//	MMAL_CONNECTION_T*			VidToSplitConn;
	...
	friend CCamera* StartCamera(int width, int height, int frame_rate, bool do_argb_conversion);
	friend void StopCamera();
};

CCamera* StartCamera(int width, int height, int frame_rate, bool do_argb_conversion=true);
void StopCamera();